...
const PremiumMasterCase = () => {
  ...
  const handleSimulateCase = () => {
    ...
    const incentivePrizes = [
      { name: 'HONDA CG FAN 160', value: 'R$ 25.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/honda cg fan.webp', bgColor: 'rgb(255, 215, 0)' },
      ...
      { name: 'S25 ULTRA', value: 'R$ 5.000,00', rarity: 'rarity-4.png', image: '/imagens/CAIXA PREMIUM MASTER/samsung s25.png', bgColor: 'rgb(255, 59, 59)' },
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (allPrizes.length > 0) {
          ...
          // Creditar todos os prêmios automaticamente para múltiplas caixas
          if (quantity > 1) {
            setTimeout(() => {
              allPrizes.forEach((prize, index) => {
                setTimeout(() => {
                  creditPrize(prize, premiumCase);
                }, index * 1000); // Delay de 1 segundo entre cada creditação
              });
            }, 2000);
          } else {
  ...
  const generateRandomPrizeSequence = () => {
    // Prêmios baixos (para credibilidade na esteira)
    const lowPrizes = [
      { name: 'R$ 2,00', value: 'R$ 2,00', rarity: 'rarity-1.png', image: '/imagens/CAIXA PREMIUM MASTER/2.png', bgColor: 'rgb(176, 190, 197)', sorteavel: false },
      { name: 'R$ 5,00', value: 'R$ 5,00', rarity: 'rarity-1.png', image: '/imagens/CAIXA PREMIUM MASTER/5.png', bgColor: 'rgb(176, 190, 197)', sorteavel: false },
      { name: 'R$ 10,00', value: 'R$ 10,00', rarity: 'rarity-1.png', image: '/imagens/CAIXA PREMIUM MASTER/10.png', bgColor: 'rgb(176, 190, 197)', sorteavel: false },
      { name: 'R$ 20,00', value: 'R$ 20,00', rarity: 'rarity-1.png', image: '/imagens/CAIXA PREMIUM MASTER/20.png', bgColor: 'rgb(176, 190, 197)', sorteavel: false },
      { name: 'R$ 100,00', value: 'R$ 100,00', rarity: 'rarity-2.png', image: '/imagens/CAIXA PREMIUM MASTER/10.png', bgColor: 'rgb(59, 130, 246)', sorteavel: false },
      { name: 'R$ 500,00', value: 'R$ 500,00', rarity: 'rarity-3.png', image: '/imagens/CAIXA PREMIUM MASTER/500.webp', bgColor: 'rgb(162, 89, 255)', sorteavel: false }
    ];

    // Prêmios altos (para incentivar depósito)
    const highPrizes = [
      { name: 'AIRPODS', value: 'R$ 2.500,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/airpods.png', bgColor: 'rgb(255, 215, 0)', sorteavel: false },
      { name: 'SAMSUNG S25', value: 'R$ 5.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/samsung s25.png', bgColor: 'rgb(255, 215, 0)', sorteavel: false },
      { name: 'IPAD', value: 'R$ 8.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/ipad.png', bgColor: 'rgb(255, 215, 0)', sorteavel: false },
      { name: 'IPHONE 16 PRO MAX', value: 'R$ 10.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/iphone 16 pro max.png', bgColor: 'rgb(255, 215, 0)', sorteavel: false },
      { name: 'MACBOOK', value: 'R$ 15.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/macbook.png', bgColor: 'rgb(255, 215, 0)', sorteavel: false },
      { name: 'HONDA CG FAN 160', value: 'R$ 25.000,00', rarity: 'rarity-5.png', image: '/imagens/CAIXA PREMIUM MASTER/honda cg fan.webp', bgColor: 'rgb(255, 215, 0)', sorteavel: false }
    ];

    // Combinar todos os prêmios (baixos + altos para exibição)
    const allPrizes = [...lowPrizes, ...highPrizes];
  ...
  const nextPrize = () => {
    if (currentPrizeIndex < wonPrizes.length - 1) {
      ...
      // Creditar o prêmio atual apenas se não foi creditado automaticamente
      const prizeKey = `${currentPremiumCase.id}-${wonPrizes[nextIndex].apiPrize.id}`;
      if (!creditedPrizes.has(prizeKey)) {
        setTimeout(() => {
          creditPrize(wonPrizes[nextIndex], currentPremiumCase);
        }, 500);
      }
    }
  };
  ...
  const creditPrize = async (prizeData = null, caseData = null) => {
    try {
      ...
      const caseInfo = caseData || currentPremiumCase;
      
      if (!caseInfo || !prize?.apiPrize) {
        console.error('Dados do prêmio não encontrados:', { caseInfo, prize });
      ...
      <main className="container mx-auto px-4 py-[-2]">
          ...
          <>
            ...
            <div className="text-center mb-8">
              ...
              <div className="flex justify-center gap-3 mb-2">
                {(user?.tipo_conta === 'afiliado_demo' ? (user?.saldo_demo || 0) : (user?.saldo_reais || 0)) >= 15.00 ? (
                  <button
                    onClick={handleOpenCase}
                    disabled={isSimulating || (user?.tipo_conta === 'afiliado_demo' ? (user?.saldo_demo || 0) : (user?.saldo_reais || 0)) < 15.00}
                    style={{background: 'rgb(14, 16, 21)', border: 'none', padding: '0px', borderRadius: '1.5rem', minWidth: '240px', cursor: (isSimulating || (user?.tipo_conta === 'afiliado_demo' ? (user?.saldo_demo || 0) : (user?.saldo_reais || 0)) < 15.00) ? 'not-allowed' : 'pointer', opacity: (isSimulating || (user?.tipo_conta === 'afiliado_demo' ? (user?.saldo_demo || 0) : (user?.saldo_reais || 0)) < 15.00) ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-start', boxShadow: 'none'}}
                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-[#0E1015] rounded-lg p-6 w-full max-w-4xl mx-4">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-white mb-2">Abrindo Caixa Premium Master</h2>
...
export default PremiumMasterCase;