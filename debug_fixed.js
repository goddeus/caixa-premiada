// ========================================
// 🔍 DEBUG CORRIGIDO - CAIXA PREMIADA
// ========================================
// Cole este código no console do navegador (F12) para debugar o sistema completo

console.log('🔍 INICIANDO DEBUG CORRIGIDO DO SISTEMA CAIXA PREMIADA...');
...
function debugApiConfig() {
  ...
  // Verificar se há configuração da API no localStorage
  const apiConfig = localStorage.getItem('apiConfig');
  if (apiConfig) {
    console.log('✅ Configuração da API encontrada:', JSON.parse(apiConfig));
  } else {
    console.log('⚠️ Nenhuma configuração da API encontrada no localStorage');
  }
...
async function debugCaseData() {
  ...
  for (const caseData of cases) {
    try {
      ...
      const response = await fetch(`${baseUrl}/cases/${caseData.id}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      console.log(`📡 Status da resposta para ${caseData.nome}:`, response.status);
...
async function debugBuyCase(caseId, caseName) {
  ...
  try {
    const response = await fetch(`${baseUrl}/cases/${caseId}/buy`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
...
async function runFullDebug() {
  ...
  // 5. Testar compra de uma caixa específica
...
window.checkApiConfig = debugApiConfig;

// ========================================
// 11. EXECUTAR DEBUG AUTOMATICAMENTE
// ========================================
...
console.log('- checkApiConfig() - Verifica configuração da API');
