...
async function testRTPValidationFinal() {
  console.log('🧪 TESTE DE VALIDAÇÃO RTP FINAL - 1.000 SIMULAÇÕES');
  console.log('==================================================\n');
  ...
  try {
    ...
    const testUser = await prisma.user.create({
      data: {
        email: `teste.validacao.final.${timestamp}@teste.com`,
        nome: `Usuário Validação RTP Final`,
    ...
    for (let i = 1; i <= 1000; i++) {
      ...
      try {
        // 1. Sortear prêmio usando o novo sistema centralizado (que fará o débito internamente)
        ...
        if (result.success) {
          ...
          if (userSession && userSession.valor_premios_recebidos >= userSession.limite_retorno) {
            roundsAfterLimit++;
    ...
    // Relatório final
    console.log('\n📋 RELATÓRIO FINAL DE VALIDAÇÃO RTP FINAL');
    console.log('==========================================\n');
