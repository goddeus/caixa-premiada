...
async function aggressiveRTPTest() {
  console.log('💀 TESTE COM RTP AGRESSIVO (30%)');
  console.log('==================================\n');
  console.log('🎯 Objetivo: Quebrar usuários com RTP mais baixo');
  console.log('💰 RTP alvo: 30% (muito mais agressivo)\n');

  try {
    // Primeiro, vamos ajustar o RTP para 30%
    console.log('⚙️ Ajustando RTP para 30%...');
    await prisma.rTPConfig.updateMany({
      data: {
        rtp_target: 30.0
      }
    });
    console.log('✅ RTP ajustado para 30%\n');

    // Criar 5 usuários de teste com R$ 20,00 cada
    const testUsers = [];
    ...
    for (let i = 1; i <= 5; i++) {
      const user = await prisma.user.create({
        data: {
          email: `teste.agressivo.${timestamp}.${i}@teste.com`,
          nome: `Usuário Agressivo ${i}`,
          saldo: 20.00,
          senha_hash: 'teste123',
    ...
    console.log('🎲 INICIANDO TESTE AGRESSIVO...\n');
    ...
    for (let userIndex = 0; userIndex < testUsers.length; userIndex++) {
      ...
      console.log(`👤 Usuário ${userIndex + 1}/5: ${user.nome}`);
    ...
    // Relatório final
    console.log('📋 RELATÓRIO FINAL - RTP AGRESSIVO (30%)');
    console.log('==========================================\n');
