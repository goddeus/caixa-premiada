...
async function testRTPValidationUltra() {
  console.log('🧪 TESTE DE VALIDAÇÃO RTP ULTRA - 1.000 SIMULAÇÕES');
  console.log('==================================================\n');
  console.log('🎯 Objetivo: Validar RTP = 0.001% com 1.000 simulações');
  console.log('📊 Verificações: RTP final ≤ 0.001%, banca não quebra, apenas R$1,00 após limite\n');

  try {
    // Ajustar RTP para 0.001% (ultra ultra agressivo)
    console.log('⚙️ Ajustando RTP para 0.001%...');
    await prisma.rTPConfig.updateMany({
      data: {
        rtp_target: 0.001
      }
    });
    console.log('✅ RTP ajustado para 0.001%\n');

    // Criar usuário de teste
    const timestamp = Date.now();
    const testUser = await prisma.user.create({
      data: {
        email: `teste.validacao.ultra.${timestamp}@teste.com`,
        nome: `Usuário Validação RTP Ultra`,
    ...
    const expectedRTP = 0.001;
    const rtpDifference = finalRTP - expectedRTP;

    // Relatório final
    console.log('\n📋 RELATÓRIO FINAL DE VALIDAÇÃO RTP ULTRA');
    console.log('==========================================\n');
    ...
    // Validação 1: RTP final ≤ 0.001%
    const rtpValid = finalRTP <= expectedRTP + 0.0001; // Margem de 0.0001%
    console.log(`${rtpValid ? '✅' : '❌'} RTP final ≤ 0.001%: ${finalRTP.toFixed(3)}% (${rtpValid ? 'PASSOU' : 'FALHOU'})`);
