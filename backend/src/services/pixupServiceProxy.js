// Proxy interno para bypass do Cloudflare
...
/**
 * Serviço de integração com Pixup usando proxy interno
 * Bypassa o Cloudflare usando conexão direta
 */
class PixupServiceProxy {
  ...
  constructor() {
    ...
    // Configurar cliente HTTP com configurações específicas
    this.client = axios.create({
      ...
      // Configurações para bypass do Cloudflare
  ...
  async authenticate() {
    try {
      ...
      console.log('[PIXUP-PROXY] Autenticando...');
      
      // Pixup usa Basic Auth no header
      const authHeader = Buffer.from(`${this.clientId}:${this.clientSecret}`).toString('base64');
      
      const response = await this.client.post(`${this.apiUrl}/v2/oauth/token`, {}, {
        headers: {
      ...
      console.log('[PIXUP-PROXY] Token obtido com sucesso');
      return this.accessToken;
      
    } catch (error) {
      console.error('[PIXUP-PROXY] Erro na autenticação:', error.response?.data || error.message);
  ...
  async createPayment({ userId, valor }) {
    try {
      console.log('[PIXUP-PROXY] Criando pagamento:', { userId, valor });
      ...
      console.log('[PIXUP-PROXY] Enviando para API:', JSON.stringify(paymentData, null, 2));
      
      // Fazer chamada para o Pixup usando URL completa
      const response = await this.client.post(`${this.apiUrl}/v2/pix/qrcode`, paymentData, {
        headers: {
          'Authorization': `Bearer ${this.accessToken}`
        }
      });
      
      console.log('[PIXUP-PROXY] Resposta da API:', JSON.stringify(response.data, null, 2));
      ...
      if (qrCode) {
        try {
          ...
          console.error('[PIXUP-PROXY] Erro ao gerar imagem do QR Code:', qrError);
        }
      }
      
      // Validar se QR Code foi extraído
      if (!qrCode) {
        console.error('[PIXUP-PROXY] ERRO: QR Code não encontrado na resposta da Pixup');
        console.error('[PIXUP-PROXY] Resposta completa:', JSON.stringify(response.data, null, 2));
      ...
      console.log(`[PIXUP-PROXY] Depósito criado: ${deposit.id} - R$ ${valorNumerico}`);
      ...
      console.error('[PIXUP-PROXY] Erro ao criar pagamento:', error);
      throw error;
    }
  }
}

module.exports = PixupServiceProxy;
