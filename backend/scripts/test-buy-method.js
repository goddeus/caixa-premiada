// TESTE DO MÉTODO BUYCASE
const axios = require('axios');

const API_BASE_URL = 'https://slotbox-api.onrender.com/api';

async function testBuyMethod() {
  console.log('🧪 TESTE DO MÉTODO BUYCASE');
  console.log('==========================');
  ...
  try {
    ...
    if (casesResponse.data.success && casesResponse.data.data.length > 0) {
      ...
      // 2. Testar endpoint de abertura com token válido
      console.log('\n🎯 2. TESTANDO ENDPOINT DE ABERTURA...');
      
      // Criar token válido com usuário existente
      const jwt = require('jsonwebtoken');
      const validToken = jwt.sign(
        { 
          userId: 'usuario-valido-123',
