...
// ===== TESTE 7: VERIFICAR ELEMENTOS DE PRÊMIO =====
function verificarElementosPremio() {
  console.log('\n🎁 === VERIFICANDO ELEMENTOS DE PRÊMIO ===');
  ...
  setTimeout(() => {
    ...
    elementosPremio.forEach((elemento, index) => {
      console.log(`   ${index + 1}. "${elemento.textContent?.trim()}" (${elemento.tagName})`);
    });
    
    // Verificar se há animações ou transições
    const elementosAnimados = document.querySelectorAll('[class*="animate"], [class*="transition"], [class*="animation"]');
    console.log(`🎬 Elementos com animação: ${elementosAnimados.length}`);
...
// ===== FUNÇÃO PRINCIPAL =====
async function executarTesteFluxoCompra() {
  console.log('🧪 EXECUTANDO TESTE DO FLUXO DE COMPRA ATUAL...');
  ...
  try {
    ...
    // Teste 6: Verificar elementos de prêmio
    verificarElementosPremio();
    ...
    setTimeout(() => {
      ...
      console.log('\n💾 Resultados salvos em window.testeFluxoCompra');
...
// ===== EXECUTAR TESTE =====
console.log('🧪 TESTE DO FLUXO DE COMPRA ATUAL PRONTO!');
console.log('📋 Execute: executarTesteFluxoCompra()');
console.log('💡 Certifique-se de estar em uma página de caixa específica');

// Executar automaticamente
executarTesteFluxoCompra();
